...
void processTouch() {
  ...
  if (readFT6236TouchLocation(&touch, 1) > 0) {
    ...
    // Wind Speed Buttons
    if (touchX > 330 && touchX < 380 && touchY > 700 && touchY < 760) {  // "+" button
      if (windSpeed < 400) { 
        windSpeed += 10;  // Increase by 10 for better control
        updateWindSpeedDisplay();
        changeWindSpeed();  // Update fan speed immediately
      }
    } 
    else if (touchX > 330 && touchX < 380 && touchY > 500 && touchY < 560) { // "-" button
      if (windSpeed > 0) { 
        windSpeed -= 10;  // Decrease by 10
        updateWindSpeedDisplay();
        changeWindSpeed();  // Update fan speed immediately
      }
    }
    ...
    // Temperature Buttons
    if (touchX > 120 && touchX < 170 && touchY > 700 && touchY < 760) {  // "+" button
      if (temperature < 90) { 
        temperature++; 
        updateTemperatureDisplay();
      }
    } 
    else if (touchX > 120 && touchX < 170 && touchY > 500 && touchY < 560) { // "-" button
      if (temperature > 0) { 
        temperature--; 
        updateTemperatureDisplay();
      }
